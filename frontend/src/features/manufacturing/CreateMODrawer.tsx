...
const schema = z.object({
  ...
  quantity_to_produce: z.coerce.number().int().positive("Quantity must be greater than zero"),
...
export function CreateMODrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <DrawerShell open={open} onClose={onClose} label={t("manufacturing.newMo")}>
        <div className="flex items-center justify-between border-b border-outline-variant p-4">
          <h3 className="text-title-sm text-on-background">{t("manufacturing.newMo")}</h3>
        ...
        <form id="mo-form" onSubmit={onSubmit} className="flex-1 space-y-5 overflow-y-auto p-4">
          <div>
            <Label htmlFor="bom_id">{t("manufacturing.billOfMaterials")}</Label>
            <select
              id="bom_id"
              defaultValue=""
              {...register("bom_id")}
            ...
            >
              <option value="" disabled>
                {loadingBoms ? t("manufacturing.loadingBoms") : t("manufacturing.selectBom")}
              </option>
              {(boms ?? []).map((b) => (
          ...
          <div>
            <Label htmlFor="quantity_to_produce">{t("manufacturing.quantityToProduce")}</Label>
            <Input id="quantity_to_produce" type="number" min="0" step="1" {...register("quantity_to_produce")} />
          ...
          <div>
            <Label htmlFor="assignee">{t("manufacturing.assigneeOptional")}</Label>
            <Input id="assignee" {...register("assignee")} placeholder="e.g. Line A" />
          </div>

          <div>
            <Label htmlFor="schedule_date">{t("manufacturing.scheduleDateOptional")}</Label>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-outline-variant bg-surface p-4">
          ...
          <Button type="submit" form="mo-form" size="sm" disabled={createMut.isPending}>
            {createMut.isPending ? t("manufacturing.creating") : t("manufacturing.createMo")}
          </Button>
        </div>
    </DrawerShell>
  );
